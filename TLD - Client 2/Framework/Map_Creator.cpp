...
void Map::Load(std::string filename, Quests* speech)
{
	std::string temp;
	...
	for (int y = 0; y < this->height; y += 1)
	{
		for (int x = 0; x < this->width; x += 1)
		{
			...
			case 8:
				this->texture->update(tile1, x * 64, y * 64);
				this->entityManager->Add("mob1", new Monster2(this, x * 64, y * 64));
				this->data[x + y * this->width] = 1;
				break;
			case 9:
				this->texture->update(tile1, x * 64, y * 64);
				this->entityManager->Add("mob2", new Monster1(this, x * 64, y * 64));
				this->data[x + y * this->width] = 1;
				break;
			}
		}
